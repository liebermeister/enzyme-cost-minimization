function ecm_check_parameter_balancing(r, r_orig, network)
...
if length(ind_change),
 display(sprintf('Changes of mu0 values (additive change > %f kK/mol',threshold_mu));
...
if length(ind_change),
display(sprintf('Changes of Keq values (fold change > %f)',threshold));
...
if length(ind_change),
display(sprintf('Changes of Kcatf values (fold change > %f)',threshold));
...
if length(ind_change),
display(sprintf('Changes of Kcatr values (fold change > %f)',threshold));
...
if length(indices), 
display(sprintf('Changes of KM values (fold change > %f)',threshold));
...
indices = indices(order(end:-1:1));
if length(indices), 
display(sprintf('Changes of KA values (fold change > %f)',threshold));
...
indices = indices(order(end:-1:1));
if length(indices), 
display(sprintf('Changes of KI values (fold change > %f)',threshold));
