...
%ECM_SAVE_MODEL_AND_DATA_SBTAB - Write SBtab file containing model and validation data for Enzyme Cost Minimization
%
% ecm_save_model_and_data_sbtab(filename, network, v, r, c_data, u_data, kinetic_data, conc_min, conc_max, met_fix, conc_fix, enzyme_cost_weights, document_name, save_single_tables,verbose)
%
...
% flux table
if length(v),      
  flux_table = sbtab_table_construct(struct('TableName','Flux', 'TableType','Quantity','TableID','Quantity','Unit','mM/s'),{'QuantityType','Reaction','Reaction:Identifiers:kegg.reaction','Value'},{repmat({'flux'},nr,1),network.actions,network.reaction_KEGGID,v});
  if save_single_tables,
...
if length(r),
  ...
  dGFE_table = sbtab_table_construct(struct('TableName','GibbsEnergyOfReaction','TableType','Quantity','Unit','kJ/mol','StandardConcentration','1mM'),{'QuantityType','Reaction','Reaction:Identifiers:kegg.reaction','Value','OriginalValue'},{repmat({'standard Gibbs energy of reaction'},nr,1),network.actions, network.reaction_KEGGID, delta_mu0, delta_mu0_orig});
  if save_single_tables,
...
if length(u_data),
  ...
  enzyme_table = sbtab_table_construct(struct('TableName','EnzymeData','TableType','Quantity','TableID','Quantity',...
...
if length(conc_min),
  constraint_table = sbtab_table_construct(struct('TableName','ConcentrationConstraint', 'TableType','Quantity','TableID','Quantity','Unit','mM'),{'QuantityType','Compound','Compound:Identifiers:kegg.compound','Concentration:Min','Concentration:Max'},{repmat({'concentration'},nm,1),network.metabolites, network.metabolite_KEGGID, conc_min, conc_max});
  if save_single_tables,
