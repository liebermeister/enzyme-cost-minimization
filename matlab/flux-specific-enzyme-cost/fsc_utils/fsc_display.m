function fsc_display(model_name,network,v,fsc_options,c,u,u_tot,up,A_forward,r)
...
figure(4); clf; 
gp = struct('arrowsize',0.05,'actstyle','none','colorbar',1,'showsign',0,'actprintnames',0,'metprintnames',1,'flag_edges',1,'colormap', ryb_colors);
gp.actvaluesmin = log10(10);
gp.actvaluesmax = log10(10000);
gp.actstyle     = 'fixed';
gp.arrowstyle   = 'none';
netgraph_concentrations(fsc_options.network_CoHid,[],log10(r.Kcatf),1,gp); 
title('log10 Forward catalytic rate constants (1/s)');

figure(11); clf; 
gp.actstyle = 'none';
gp.metvaluesmin = log10(fsc_options.conc_min_default);
gp.metvaluesmax = log10(fsc_options.conc_max_default);
netgraph_concentrations(network,log10(c.init),[],1,gp); title('Initial state: log10 Concentrations (mM)');
...
figure(8); clf
gp = struct('arrowstyle','none','colorbar',1,'showsign',0,'actprintnames',0,'flag_edges',1,'colormap', ryb_colors);
gp.metvaluesmin = log10(fsc_options.conc_min_default);
gp.metvaluesmax = log10(fsc_options.conc_max_default);
...
figure(9); clf
gp = struct('arrowstyle','none','colorbar',1,'showsign',0,'actprintnames',1,'flag_edges',1,'colormap', ryb_colors);
...
for it = 1:length(my_fsc_scores),
  ...
  h(it)     = plot(xtick,c.(fsc_score)(ind_show_met,:) * exp(0.03*it),'-','MarkerSize',20); %  * exp(0.001*it)
end
line_colors(h,'ryb_colors');
...
for it = 1:length(my_fsc_scores),
  fsc_score = my_fsc_scores{it};
  h(it)     = plot(xtick,[0; -cumsum(v .* A_forward.(fsc_score))] + (0.002*it),'-'); %  
end
line_colors(h,'ryb_colors');

legend(my_fsc_scores,'Location','NorthEastOutside','FontSize',10);
axis tight; set(gca,'XTick',xtick,'XTickLabel',[{''};strrep(network.actions,'_', ' ')]);
my_xticklabel([],[],[],10);
title('Comparison: cumulative entropy production (v * A)');

% --------------------------------------
% c over KM

figure(20); subplot('Position',[0.15 0.05 0.8 0.7]);
c_over_KM = repmat(c.init',length(network.actions),1)./r.KM;
c_over_KM(r.KM==0) = nan;
im(log10(c_over_KM(:,ind_show_met)),[-3,3],network.actions,network.metabolites(ind_show_met)); colormap([0 0 0; ryb_colors]); colorbar
my_xticklabel
...
for it = 1:length(fsc_options.fsc_scores),
  ...
  ug_meth   = up.(fsc_score)(fsc_options.ind_scored_enzymes);
  ug_meth   = ug_meth/sum(ug_meth);
  ug        = [ug, ug_meth];
end
bar(ug(ind_genes_scored,:)','stacked');
axis([0,4+length(fsc_options.fsc_scores), 0, 1.02]); colormap(ryb_colors); 
legend(gene_names(ind_genes_scored),'Fontsize',8,'Location','EastOutside')
...
gpp = struct('actprintnames',1,'arrowstyle','none','showsign',0,'colormap',ryb_colors,'text_offset',[0.03,0]);
gpp.actnames = gene_names;
netgraph_concentrations(network,[],column(mmm),1,gpp); 
...
if fsc_options.print_graphics,
  if isfield(fsc_options,'psfile_dir'),
    ...
    print([ model_name '_' fsc_options.run_id '_kcat.eps'],'-f4','-depsc');
...
for it_method = 1:length(fsc_options.fsc_scores),
  ...
  my_c = c.(fsc_score);
  my_A_forward = A_forward.(fsc_score);
  my_up = up.(fsc_score);
